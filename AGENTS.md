...
- **Send/freeze does NOT hard-require a complete checklist.** The QA wizard (`runScopeChecks` in `index.html`) blocks outputs only on RED items; photos are RED only at **zero** (≥1 is fine), and <5 photos / unfinished video are AMBER (acknowledge-only). So a scope can freeze with an incomplete checklist. `_getScopeErrors` (~13210, the "≥1 site photo" rule) is dead code — no call site. Consider a harder pre-freeze gate as a follow-up.
